...
export interface Club {
  ...
  /**
   * Город клуба.
   * В админке хранится человекочитаемое название (например, «Воронеж»),
   * но в документации также используется машинное имя (например, `voronezh`).
   */
  city: string;
  ...
  pricing?: ClubPricing;
  /**
   * Telegram-аккаунт администратора клуба (например, 'admin_username' или '@admin_username').
   * Используется ботом для отправки уведомлений о новых бронированиях и оплатах.
   */
  telegramAdmin?: string;
  /**
   * Способ интеграции онлайн-оплат:
   * - 'yookassa' — создаётся платёж в ЮKassa
   * - 'bank_account' — клиенту отправляется ссылка на оплату по расчётному счёту
   */
  paymentIntegration?: 'yookassa' | 'bank_account';
  /**
   * Ссылки на оплату по расчётному счёту для разных типов услуг.
   * Обычно используется как paymentLinks['regular'] для регулярных броней.
   */
